...
import { addStaff, editStaff, removeStaff } from "../../../lib/staffActions"
...
export default async function StaffPage({
  ...
  const errorText = errorCode
    ? errorCode === "identifier"
      ? t(
          {
            ar: "اكتب رقم الموبايل أو ID التطبيق.",
            en: "Enter a mobile or App ID.",
          },
          lang,
        )
      : errorCode === "notfound"
        ? t(
            {
              ar: "مفيش حساب بالبيانات دي — لازم الموظّف يسجّل في التطبيق الأول.",
              en: "No account with these details — the staff member must register first.",
            },
            lang,
          )
        : errorCode === "self"
          ? t(
              { ar: "مينفعش تضيف نفسك.", en: "You can't add yourself." },
              lang,
            )
          : t(
  ...
  const inputCls =
    "block w-full rounded-lg border border-black/10 px-3 py-2 text-sm text-slate-800 outline-none focus:border-brand-400"
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <input type="text" name="identifier" required className={inputCls} />
          </div>
          <div>
            <label className="mb-1 flex items-center gap-1.5 text-sm font-medium text-slate-700">
      ...
      <div className="mb-3 text-sm font-semibold text-slate-700">
        {t({ ar: "الموظفون الحاليون", en: "Current staff" }, lang)} ({staff.length})
      </div>
      {staff.length === 0 ? (
          ...
          {staff.map((s) => (
            ...
            >
              ...
              <details className="mt-3">
                <summary className="inline-flex cursor-pointer items-center gap-1.5 text-sm font-medium text-brand-600 hover:text-brand-700">
                  <Pencil className="h-4 w-4" />
                  {t({ ar: "تعديل", en: "Edit" }, lang)}
                </summary>
                <form
                  action={editStaff}
                  className="mt-3 space-y-4 border-t border-black/5 pt-4"
                >
                  <input type="hidden" name="membershipId" value={s.id} />
                  ...
                  <div>
                    ...
                    <PermissionsGrid
                      groups={PROVIDER_PERMISSION_GROUPS}
                  ...
                  >
                    ...
                    {t({ ar: "حفظ التعديل", en: "Save changes" }, lang)}
                  </button>
                </form>
              </details>
            </div>
