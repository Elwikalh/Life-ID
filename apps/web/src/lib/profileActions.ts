...
export async function saveProfile(formData: FormData) {
  ...
  const consultationFee = Number.isFinite(parsedFee)
  ...
  try {
    await prisma.user.upsert({
      where: { id: u.id },
      update: { specialty, bio, consultationFee, phone },
      create: {
        id: u.id,
        role,
        fullName,
    ...
    // لو رقم الهاتف متكرر (unique) أو أي خطأ، نتجاهله بدل ما نكسر الحفظ
    try {
      await prisma.user.upsert({
        where: { id: u.id },
